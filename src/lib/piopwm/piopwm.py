...
class PIOPWM:
  ...
  def __init__(self, id, pin, mode=NORMAL, in_pin=None):
    freq = machine_freq()
    if (mode == NORMAL):
      self.sm = StateMachine(id, prog=pwm_program, freq=freq, sideset_base=Pin(pin))
    elif (mode == WITH_PIN):
      if (in_pin == None):
        raise Exception("Define in_pin")
      self.sm = StateMachine(id, prog=pwm_with_pin_program, sideset_base=Pin(pin), in_base=Pin(in_pin), freq=freq)
    elif (mode == WITH_PIN_INVERTED):
      if (in_pin == None):
        raise Exception("Define in_pin")
      self.sm = StateMachine(id, prog=pwm_with_pin_program_inverted, sideset_base=Pin(pin), in_base=Pin(in_pin), freq=freq)
    elif (mode == DOUBLE_PIN):
      pin_1 = pin
      self.sm = StateMachine(id, prog=pushpull_program, sideset_base=Pin(pin_1), freq=freq)
    else:
