...
class Program(UIListProgram, WithExp):
    ...
    def render_period(self):
        return ns_to_str(ticks_to(self.pwm.get_high() + self.pwm.get_low() * 2))

    def render_overlap(self):
        return ns_to_str(ticks_to(self.pwm.get_low()))

    def update_period(self, event):
        new_value = self.get_value_by_exp(
            get_period(), self.dir_to_inc(event), "period")
        store.set("period", max(get_overlap()*2, new_value))
        self.load_params()

    def update_overlap(self, event):
        new_value = self.get_value_by_exp(
            get_period(), self.dir_to_inc(event), "overlap")
        store.set("overlap", max(0, min(int(get_period()/2), new_value)))
        self.load_params()

    def load_params(self):
        self.pwm.set_params((get_period(), get_overlap()))
