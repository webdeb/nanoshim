...
class Pwm(UIListProgram):
  ...
  def __init__(self, on_exit):
    ...
    self.items = [
      ...
      # """
      {
        "text": [["F2 Phase:", self.get_exp("f2_low")], self.get_low(F2)],
        "handle_change": lambda event: self.change_low(F2, event),
      },
      {
        "text": [["High:", self.get_exp("f2_high")], lambda: str(self.piopwm.get_high(F2))],
        "handle_change": lambda event: self.change_high(F2, event),
      },
